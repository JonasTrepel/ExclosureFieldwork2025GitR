...
p_plot <- estimates %>%
  ...
  geom_jitter(data = dt %>%
                ...
                left_join(rts) %>% 
                unique(), aes(x = ln_rr, y = clean_response, fill = biome), alpha = 0.75, height = 0.1, shape = 21) +
  ...
  scale_fill_scico_d(palette = "bamako", direction = -1, begin = 0.5, end = 0.9) +
  geom_pointrange(aes(x = estimate, xmin = ci_lb, xmax = ci_ub, y = clean_response, color = sig), linewidth = 1.3, alpha = 0.9) +
  labs(title = "Site-Specific Estimates", y = "Response", x = "Log-Response Ratio", fill = "Biome", #fill = expression(NPP~(kg~C~ha^{-1}~yr^{-1})),
       color = "") +
  theme_minimal() +
