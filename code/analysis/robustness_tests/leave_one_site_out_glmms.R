...
for(setup in unique(setups)){
  
dt_sub <- dt %>% 
  filter(!setup_id %in% c(setup))

estimates_sub <- data.table()
for(response in unique(guide$response_name)){
  ...
  estimates_sub <- rbind(estimates_sub, tmp_est) %>% as.data.table()
...
estimates_sub <- estimates_sub %>%
  mutate(excluded_setup = setup)
  
estimates <- rbind(estimates_sub, estimates)

print(paste0(setup, "removed done"))
...
rts <- estimates %>% 
  dplyr::select(response_name, response_tier, clean_response_tier, clean_response) %>% 
  unique()
...
p_plot <- estimates %>%
  ...
  ggridges::geom_density_ridges_gradient(aes(x = estimate, y = clean_response,
                      fill = after_stat(x)), alpha = 0.9) + 
...
library(patchwork)
p_comb <- p_plot | p_site +
  #plot_layout(widths = c(1, 1)) +
  plot_annotation(
    title = "Estimate distribution of Leave One Site Out Sensitivity Analysis",
    theme = theme(
...
ggsave(plot = p_comb, "builds/plots/supplement/leave_one_site_out_glmm_estimates.png", dpi = 600, height = 6, width = 11)
