...
guide <- dt %>% 
  mutate(formula = case_when(
    ...
    ) ~ "1 + (1 | setup_id /exclosure_id/cluster_id)",
    #site_level
...
for(response in unique(guide$response_name)){
  ...
  if(sca == "plot"){
    ...
    dt_mod <- dt %>% 
      filter(response_name %in% c(response)) %>% 
      dplyr::select(setup_id, ln_rr, biome) %>% 
      unique() %>% 
      filter(!is.na(ln_rr)) %>% 
      filter(!is.infinite(ln_rr)) %>% 
      filter(biome == "Savanna")
...
estimates <- estimates %>% 
 ...
 rts <- estimates %>% dplyr::select(response_name, response_tier, clean_response_tier, clean_response) %>% unique()
...
p_site <- estimates %>%
  ...
  geom_pointrange(aes(x = estimate, xmin = ci_lb, xmax = ci_ub, y = clean_response,
                      color = sig), linewidth = 1.3, alpha = 0.9) + 
  labs(title = "Site-Scale Savanna Only", y = "", x = "Log-Response Ratio", fill = "Log-\nResponse\nRatio", color = "") +
  theme_minimal() +
  xlim(x_min, x_max) +
  scale_y_discrete(limits = rev) +
...
estimates %>% 
  filter(response_name == "plant_richness_site")

ggsave(plot = p_site, "builds/plots/supplement/glmm_estimates_site_scale_savannas_only.png", dpi = 600, height = 5, width = 8)
